...
<section style="padding:16px;font:14px/1.4 system-ui">
<h1>CN‑SRS</h1>
<p>Cards geladen: {cards.length} • Fällige: {dueCount}</p>
{#if view === 'learn'}
{#if current}
<div style="margin:12px 0;padding:12px;border:1px solid #ddd">
<div style="font-size:28px">{current.hanzi}</div>
<div style="opacity:.8">{current.pinyin}</div>
<button on:click={() => speakHanzi()}>Aussprache</button>
<button on:click={() => showAnswer = !showAnswer}>
{showAnswer ? 'Antwort verbergen' : 'Antwort anzeigen'}
</button>
{#if showAnswer}
<div style="margin-top:8px">{current.meaning_de}</div>
{/if}
<div style="margin-top:8px">
<button on:click={() => rate('again')}>Wiederholen</button>
<button on:click={() => rate('hard')}>Schwer</button>
<button on:click={() => rate('good')}>Gut</button>
<button on:click={() => rate('easy')}>Einfach</button>
</div>
</div>
{:else}
<p>Keine fälligen Karten. Wechsle zu „Verwalten“, um Karten hinzuzufügen.</p>
{/if}
{:else if view === 'manage'}
<p>Verwalten-Ansicht (hier würdest du deine Formulare/Listen einfügen).</p>
{/if}

<div style="margin-top:16px">
<button on:click={() => view = 'learn'} disabled={view==='learn'}>Lernen</button>
<button on:click={() => view = 'manage'} disabled={view==='manage'}>Verwalten</button>
<button on:click={() => makeAllDueNow()}>Alle fällig</button>
</div>
</section>